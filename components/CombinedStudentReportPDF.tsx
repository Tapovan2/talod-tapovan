...
interface CombinedStudentReportProps {
  month: string;
  year: string;
  standard: string;
  className: string;
...
// Component to render a single student report
const StudentReport = ({
  student,
  month,
  year,
  ...
  const processedSubjects = student.subjects.map((subject) => {
    const convertedScores = subject.examDetails.map((exam) => {
      if (exam.score === null || exam.score === "AB") return "AB";
      if (typeof exam.score === "string") return exam.score;
  ...
  return (
    <>
      ...
      <View style={styles.attendanceTable}>
        <View style={styles.attendanceRow}>
          ...
          <View style={styles.attendanceValue}>
            <Text>{student.attendance.totalDays}</Text>
          </View>
        </View>
        <View style={[styles.attendanceRow, { borderBottomWidth: 0 }]}>
          ...
          <View style={styles.attendanceValue}>
            <Text>{student.attendance.presentDays}</Text>
          </View>
        </View>
      </View>
      ...
      {!isLastStudent && <View style={styles.pageBreak} />}
    </>
  );
};

export function CombinedStudentReportPDF({
  month,
  year,
  standard,
  className,
  students,
}: CombinedStudentReportProps) {
  return (
    <Document>
      {students.map((student, index) => (
        <Page key={student.rollNo} size="A4" style={styles.page}>
          <StudentReport
            student={student}
            month={month}
            year={year}
